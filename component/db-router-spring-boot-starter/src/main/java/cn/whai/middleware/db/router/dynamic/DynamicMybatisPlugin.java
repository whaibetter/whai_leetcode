package cn.whai.middleware.db.router.dynamic;
...
//拦截了 StatementHandler 类的 prepare 方法
public class DynamicMybatisPlugin implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        //获取自定义注解判断是否进行分表操作
        String id = mappedStatement.getId(); // 方法的权限定方法名
        String className = id.substring(0, id.lastIndexOf("."));

        // 类名称
        Class<?> clazz = Class.forName(className); // 加载类
        DBRouterStrategy dbRouterStrategy = clazz.getAnnotation(DBRouterStrategy.class);
        if (null == dbRouterStrategy || !dbRouterStrategy.splitTable()) { // 不分表
        ...
        log.info("分表前表名:{}", tableName);

        if (tbKey != null) {
            tableName = tableName + "_" + tbKey;
        }
        String replaceSql = matcher.replaceAll(tableName);
