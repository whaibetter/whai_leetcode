package cn.whai.middleware.db.router.strategy.impl;

import cn.whai.middleware.db.router.DBContextHolder;
 ...
 * @description: 哈希路由
 */
@ConditionalOnProperty(value = "db-router.strategy", havingValue = "hashCode")
public class DBRouterStrategyHashCode implements IDBRouterStrategy {
    ...
    /**
     *
     * 根据路由字段，计算Table和DB的位置，封装到ThreadLocal的DBContextHolder中便于后面定位
    ...
    public void doRouter(String dbKeyAttr) {
        ...
        int tbIdx = idx - dbRouterConfig.getTbCount() * (dbIdx - 1); //  idx 减去已经分配给前 dbIdx - 1 个数据库的表数量，得到当前数据库中的表索引 tbIdx。
